...
from app.src.preprocess_text import preprocess_text, prepare_multilabel_data, get_word2vec_features, Pre_Processamento, labels_em_cols, medidas_multilabel, extrair_entrevistado
...
sheet_name = "Detrator" # TREINO  |  Completo_semNS
...
train_df = data_multilabel.loc[(data_multilabel['NPS'] != "Detrator_TESTE") & (data_multilabel['ONDA'] != "Ago/25")] 
test_df = data_multilabel.loc[(data_multilabel['NPS'] == "Detrator_TESTE")] # Categoria NPS e de Julho/25
test_df_2 = data_multilabel.loc[(data_multilabel['ONDA'] == "Ago/25")] # Categoria NPS e de Agosto/25

bancos_teste = [test_df, test_df_2]
...
test_df['resposta'] = test_df['resposta'].fillna('')
...
# xgb_multi = OneVsRestClassifier(XGBClassifier(
#                 booster="gblinear",     # modelo linear regularizado (L1/L2) — em texto costuma rivalizar/superar RL
#                 learning_rate=0.05,     # Menor (0.02–0.1) pede mais árvores; costuma generalizar melhor
#                 n_estimators=400,       # Mais árvores = mais capacidade (e risco de overfit)
#                 reg_alpha=1.0,          # L1 ajuda a esparsidade 0–5 (até 10)
#                 reg_lambda=3.0,         # L2 ajuda a esparsidade 0–5 (até 10)
#                 random_state=42,
...
xgb_multi = OneVsRestClassifier(XGBClassifier(
                    ...
                    tree_method="hist",
                    learning_rate=0.05,
                    n_estimators=1000,            # com early stopping
                    max_depth=4,                  # raso p/ texto
                    min_child_weight=5,
                    gamma=1.0,
                    subsample=0.8,
                    colsample_bytree=0.3,         # amostra poucas colunas
                    reg_alpha=2.0,
                    reg_lambda=6.0,
                    random_state=42,
                    eval_metric="aucpr"
...
# xgb_multi = OneVsRestClassifier(GradientBoostingClassifier(n_estimators=300, loss='exponential'))
...
preds_ml = [estimator.predict_proba(X_test) for estimator in xgb_multi.estimators_]
probs_pos = []
for p in preds_ml:
...
pred_df_ml = pd.DataFrame(preds_topk, columns=[f"Pred_{col}" for col in labels])
result_df_ml = pd.concat([test_df.reset_index(drop=True), pred_df_ml], axis=1)
...
previsao = xgb_multi.predict(X_test)
...
# result_df_ml.to_excel("C:/PROJETOS/Classificacao de Textos/Predicoes_modelo/Resultado_31TAGs_C3_MaxFeatures_50.xlsx", index=False)
